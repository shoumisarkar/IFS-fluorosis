...
#   num_cores <- min(2, (detectCores() - 1))  # Use one less than total cores
...
# 
#   # Print progress
#   print("(Presence) Jackknife completed")
# 
...
get_JK_corrmat_pres <- function(dat_pres, init_pres_coef, parallel = TRUE) {
  ...
  if (parallel) {
    num_cores <- min(8, detectCores() - 1)  # Use at most 8 cores
    cl <- makeCluster(num_cores)
    clusterExport(cl, c("dat_pres", "tooth_zone_comb", "inv_logit", "init_pres_coef", 
                        "get_tooth_index", "get_zone_index", "epsilon"), envir = environment())
    
    results <- parLapply(cl, unique(dat_pres$SUBJECT_ID), function(id) {
      dat_pres_JK <- dat_pres[dat_pres$SUBJECT_ID != id, ]
      dat2 <- dat_pres_JK
      dat2$Wp <- dat_pres_JK$FRI_Score
      dat2$mu <- pmax(pmin(inv_logit(init_pres_coef[1] + rowSums(t(t(dat_pres_JK[, -c(1, 2)]) * init_pres_coef[-c(1)]))), 1 - epsilon), epsilon)
      dat2$var <- dat2$mu * (1 - dat2$mu)
      ...
      for (i in 1:nrow(tooth_zone_comb)) {
        for (j in i:nrow(tooth_zone_comb)) {
          if (i != j) {
            ...
            ind1 <- dat2$Tooth == tooth1 & dat2$Zone == zone1
            ind2 <- dat2$Tooth == tooth2 & dat2$Zone == zone2
            temp <- mean(outer(dat2$r[ind1], dat2$r[ind2], "*"))
            cov_mat[i, j] <- cov_mat[j, i] <- phi * temp
          }
        }
      }
    ...
    results <- lapply(unique(dat_pres$SUBJECT_ID), function(id) {
      dat_pres_JK <- dat_pres[dat_pres$SUBJECT_ID != id, ]
      dat2 <- dat_pres_JK
      dat2$Wp <- dat_pres_JK$FRI_Score
      dat2$mu <- pmax(pmin(inv_logit(init_pres_coef[1] + rowSums(t(t(dat_pres_JK[, -c(1, 2)]) * init_pres_coef[-c(1)]))), 1 - epsilon), epsilon)
  ...
  JK_corr_mat_pres[JK_corr_mat_pres > 1] <- 1
  JK_corr_mat_pres[JK_corr_mat_pres < -1] <- -1
  JK_corr_mat_pres <- (JK_corr_mat_pres + t(JK_corr_mat_pres)) / 2
  diag(JK_corr_mat_pres) <- 1
  JK_corr_mat_pres[is.nan(JK_corr_mat_pres)] <- 0
...
get_JK_corrmat_sev <- function(dat_sev, init_sev_coef, parallel = TRUE) {
  ...
  JK_corr_mat_sev <- matrix(0, nrow = 16, ncol = 16)
  ...
  if (parallel) {
    num_cores <- min(8, detectCores() - 1)  # Use at most 8 cores
    cl <- makeCluster(num_cores)
    clusterExport(cl, c("dat_sev", "tooth_zone_comb", "inv_logit", "init_sev_coef", 
                        "get_tooth_index", "get_zone_index", "epsilon"), envir = environment())
    
    results <- parLapply(cl, unique(dat_sev$SUBJECT_ID), function(id) {
      dat_sev_JK <- dat_sev[dat_sev$SUBJECT_ID != id, ]
      dat2 <- dat_sev_JK
      dat2$Ws <- as.numeric(dat_sev_JK$FRI_Score)
      
      dat2$cusum1 <- pmax(pmin(inv_logit(init_sev_coef[1] + rowSums(t(t(dat_sev_JK[, -c(1, 2)]) * init_sev_coef[-c(1,2)]))), 1 - epsilon), epsilon)
      dat2$cusum2 <- pmax(pmin(inv_logit(init_sev_coef[2] + rowSums(t(t(dat_sev_JK[, -c(1, 2)]) * init_sev_coef[-c(1,2)]))), 1 - epsilon), epsilon)
      
      dat2$pi1 <- dat2$cusum1
      dat2$pi2 <- dat2$cusum2 - dat2$cusum1
      dat2$pi3 <- 1 - dat2$cusum2
      
      dat2$r <- (dat2$Ws - dat2$pi1) / sqrt(dat2$pi1 * (1 - dat2$pi1))
      dat2$Tooth <- get_tooth_index(dat_sev_JK)
      dat2$Zone <- get_zone_index(dat_sev_JK)
      
      phi <- 1 / (sum(dat2$r^2) / (nrow(dat2) - ncol(dat_sev_JK[, -c(1, 2)])))
    ...
    results <- lapply(unique(dat_sev$SUBJECT_ID), function(id) {
      dat_sev_JK <- dat_sev[dat_sev$SUBJECT_ID != id, ]
      dat2 <- dat_sev_JK
      dat2$Ws <- as.numeric(dat_sev_JK$FRI_Score)
  ...
  JK_corr_mat_sev[JK_corr_mat_sev > 1] <- 1
  JK_corr_mat_sev[JK_corr_mat_sev < -1] <- -1
  JK_corr_mat_sev <- (JK_corr_mat_sev + t(JK_corr_mat_sev)) / 2
  diag(JK_corr_mat_sev) <- 1
  JK_corr_mat_sev[is.nan(JK_corr_mat_sev)] <- 0
...
fit_GEE_combined <- 
  function(dat = IFS_dat, kappa = 1, corstr_pres = "independence", 
    ...
    # if(corstr_pres != "jackknifed" && corstr_sev != "jackknifed"){
    #   n_cores = min(16, detectCores() - 1) #can use more cores, as no JK matrix calculation to use them on
    # }
    ...
    if(reuse==T) #if rho_pres or JK_corr_mat_pres is provided
    {
      ...
      if (corstr_pres == "exchangeable") {
        ...
        JK_corr_mat_pres <- get_JK_corrmat_pres(dat_pres = dat_pres, init_pres_coef = sep_pres_coef, parallel = F)
        rownames(JK_corr_mat_pres) <- colnames(JK_corr_mat_pres) <- tooth_zone_comb$combination
      }
    ...
    if(reuse==T) #if rho_sev or JK_corr_mat_sev is provided
    {
      ...
      }else if(n_sev_categ==2)
      {
        ...
        if (corstr_sev == "exchangeable") {
          ...
          JK_corr_mat_sev <- get_JK_corrmat_pres(dat_pres = dat_sev, init_pres_coef = sep_sev_coef[-c(2)], parallel = F)
          rownames(JK_corr_mat_sev) <- colnames(JK_corr_mat_sev) <- tooth_zone_comb$combination
        } 
      }
