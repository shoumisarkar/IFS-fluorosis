...
write_table_to_excel <- function(CI_list, corstr_sev, conf_level=95) {
  wb <- createWorkbook()
...
create_summary_table = function()
{
  ...
  for(age in ages)
  {
    ...
    temp_df  = data.frame(Variable=rownames(se_sev_list$age9),
                          ...
                          Std_SE = format(round(preJS_SE[[age_ind]]$SE, 3), nsmall=3),                          JS_Estimate = format(round(postJS_stdcoef_sev_list[[age_ind]]$Estimates, 3), nsmall=3),
                          JS_Bias = 
                            format(round( apply(stdcoefsJS_BS_list[[age_ind]], MARGIN = 1, FUN = function(x) mean(x, na.rm = TRUE)) -
                                            postJS_stdcoef_sev_list[[age_ind]]$Estimates , digits = 3), nsmall=3),
