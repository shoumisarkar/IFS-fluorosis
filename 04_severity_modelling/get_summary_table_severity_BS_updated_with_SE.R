...
create_summary_table = function(level=95)
{
  ...
  for(age in ages)
  {
    ...
    temp_df  = data.frame(Variable=rownames(se_sev_list$age9),
                          ...
                          Std_Estimate=round((coef_sev_list[[age_ind]])$Estimates/se_sev_list[[age_ind]]$SD, 3),
...
write_table_to_excel(summary_90, corstr_sev, 90)
...
write_table_to_latex <- function(my_list, corstr_sev, conf_level) {
  ...
  #cat("\\section*{", name, "}\n")
  cat("\\begin{table}[ht]\n")
  cat("\\label{ch4:table:sep:sev:", corstr_sev, "}\n", sep="")
  cat("\\centering\n")
