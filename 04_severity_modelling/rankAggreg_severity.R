...
ages = c(9, 13, 17, 23)
corstrs_sev = c("independence", "exchangeable", "ar1", "jackknifed")
...
for(age in ages)
{
  ...
  #colnames(age_spec_mat) = corstrs_sev
...
write_table_to_latex <- function(all_outputs) {
  ...
  cat("\\scriptsize\n")
  cat("\\item B.$c$ denotes the model. ``B'' indicates the separate severity model, $c$ indicates the working correlation structure: $c=1$ for \\\\ independence, $c=2$ for exchangeable, $c=3$ for AR(1), $c=4$ for jackknifed. Lower ranks correspond to lower SE of the \\\\\ James-Stein adjusted standardized estimate.\n")
  cat("\\end{tablenotes}\n")
