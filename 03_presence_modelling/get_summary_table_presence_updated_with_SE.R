...
write_table_to_excel <- function(CI_list, corstr_pres, conf_level) {
  wb <- createWorkbook()
...
process_age_data <- function(age, B, corstr_pres, get_CI, vars) {
  ...
  for (b in 1:B) {
    ...
    if (file.exists(fn_stdcoef_pres)) {
      ...
      stdcoefs_BS_mat[, b] <- std_coef_pres$`Standardized Estimate`[-c(1)]  # Adjusted to remove first row
    }
  }
...
#vars = rownames(std_coef_pres)[-c(1:2)]
...
write_table_to_latex <- function(my_list, corstr_pres, conf_level) {
  ...
  #cat("\\section*{", name, "}\n")
  cat("\\begin{table}[ht]\n")
  cat("\\label{ch4:table:sep:pres:", corstr_pres, "}\n", sep="")
  cat("\\centering\n")
...
write_table_to_latex(summary_95, corstr_pres, 95)
