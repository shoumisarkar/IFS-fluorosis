...
write_table_to_excel <- function(CI_list, corstr_pres, conf_level=95) {
  wb <- createWorkbook()
...
# Main logic
corstr_pres <- "jackknifed" #"exchangeable" #"ar1" #"independence"
ages <- c(9, 13, 17, 23)
B <- 120
...
create_summary_table = function()
{
  ...
  for(age in ages)
  {
    ...
    temp_df  = data.frame(Variable=rownames(se_pres_list$age9),
                          ...
                          Std_Estimate=format(round((coef_pres_list[[age_ind]])$Estimates/se_pres_list[[age_ind]]$SD, 3), nsmall=3),
                          ...
                          JS_Bias = 
                            format(round( apply(stdcoefsJS_BS_list[[age_ind]], MARGIN = 1, FUN = function(x) mean(x, na.rm = TRUE)) -
                                            postJS_stdcoef_pres_list[[age_ind]]$Estimates, digits = 3 ), nsmall=3),
                          ...
                          JS_MSE = format(round( (apply(stdcoefsJS_BS_list[[age_ind]], MARGIN = 1, FUN = function(x) mean(x, na.rm = TRUE)) -
                                                    postJS_stdcoef_pres_list[[age_ind]]$Estimates)^2 + postJS_SE[[age_ind]]$SE, 3), nsmall=3),
                          JS_CI=postJS_CI_int)
...
summary_95 = create_summary_table()
...
write_table_to_latex(summary_95, corstr_pres)
