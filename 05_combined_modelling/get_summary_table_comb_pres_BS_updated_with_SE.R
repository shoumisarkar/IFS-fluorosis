...
# Main logic
corstr_pres <- "jackknifed" #"ar1"
corstr_sev = "jackknifed" #"ar1" 
ages <- c(9, 13, 17, 23)
B <- 120
...
preJS_SE = lapply(stdcoefs_BS_list, get_SE, var=vars) #this is the SE for the stdcoefs_JS_BS_list
...
create_summary_table = function(level=95)
{
  ...
  for(age in ages)
  {
    ...
    temp_df  = data.frame(Variable=rownames(se_pres_list$age9),
                          ...
                          Std_Estimate=round((coef_pres_list[[age_ind]])$Estimates/se_pres_list[[age_ind]]$SD, 3),
                          ...
                          JS_Estimate = format(round(postJS_stdcoef_pres_list[[age_ind]]$Estimates, 3), nsmall=3),
...
write_table_to_latex <- function(my_list, corstr_pres, corstr_sev, conf_level) {
  ...
  caption_text <- paste0("Presence estimates from models C.", corstr_pres_index, ".", corstr_sev_index,
  ...
  #cat("\\section*{", name, "}\n")
  cat("\\begin{table}[ht]\n")
  cat("\\label{ch4:table:comb:pres:", corstr_sev, "}\n", sep="")
  cat("\\centering\n")
  ...
  for (name in names(my_list)) {
    ...
    cat("Variable & Estimate & \\makecell{Standardized\\\\Estimate} & \\makecell{SE\\\\(Standardized\\\\Estimate)} & \\makecell{James-Stein\\\\Estimator} & \\makecell{SE\\\\(James-Stein\\\\Estimator)} & \\makecell{95\\% CI\\\\(James-Stein\\\\Estimator)} \\\\\n")
    
    # Print the table body without column names
    print(xtable(my_list[[name]], align = c("r", "c", "c", "c", "c", "c", "c", "l")), 
