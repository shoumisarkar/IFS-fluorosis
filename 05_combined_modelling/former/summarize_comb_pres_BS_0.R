...
for(age in ages)
{
    ...
    
  {
    ...
    obj_stdcoef_pres = load(fn_stdcoef_pres); assign("std_coef_pres", get(obj_stdcoef_pres))
    
    stdcoefs_BS_mat[,b] = std_coef_pres$`Standardized Estimate`[-c(1:4)]
  }
...
filename_stdcoefs_BS_list = paste0("W:/somnath.datta/shoumisarkar/Fluorosis/Results/05a_combined_presence_modelling/preJS_pres_stdcoefBS_",
                                   corstr_pres, ",", corstr_pres, ".RData")
...
get_CI = function(confidence_level=0.95, BS_dat)
{
  ...
  d = data.frame(Variable=rownames(std_coef_pres)[-c(1:4)], 
...
saveWorkbook(wb_95, paste0("W:/somnath.datta/shoumisarkar/Fluorosis/Results/05a_combined_presence_modelling/pres_95_CI_",
                           corstr_pres, ",", corstr_pres, ".xlsx"), overwrite = TRUE)
...
saveWorkbook(wb_90, paste0("W:/somnath.datta/shoumisarkar/Fluorosis/Results/05a_combined_presence_modelling/pres_90_CI_",
                           corstr_pres, ",", corstr_pres, ".xlsx"), overwrite = TRUE)
