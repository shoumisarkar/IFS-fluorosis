...
if(all(c(1,2) %in% unique(temp$FRI_Score)))
{
  ...
  for(ID in c( (chunk*1000) : ((chunk+1)*1000) ) )
  {
    ...
    if(all(c(1,2) %in% unique(MC_dataset_JK$FRI_Score)))
    {
      ...
      #Save the coef_sev_JK dataframe obtained so far, in case the job runs out of time before it gets to N=10,000
      
      setwd(paste0("/blue/somnath.datta/shoumisarkar/Fluorosis/Results/06_simulation/N", N, "/", mode, "/",
                   corstr_pres, ",", corstr_sev, "/age", age))
      
      save(coef_pres_JK, file = paste0("coef_pres_JK_MC_", mc_seed, "_chunk_", chunk,".Rdata") )
      save(coef_sev_JK, file = paste0("coef_sev_JK_MC_", mc_seed, "_chunk_", chunk, ".Rdata") )
