...
if(file.exists(path))
{
  ...
  true_params[5] = assumed_beta #Set Avg_homeppm to 10
  ...
  if(all(c(1,2) %in% unique(temp$FRI_Score))){
    ...
    coef_sev_BS = matrix( , nrow=length(true_params), ncol=B)
    rownames(coef_sev_BS) = names(true_params)
    rho_sev_BS = c()
      ...
      if(all(c(1,2) %in% unique(bs_dataset$FRI_Score)))
      {
        ...
        tryCatch({
          ...
          bs_output <- withTimeout({
            fit_GEE_severity(dat = bs_dataset, corstr_sev = corstr_sev, kappa = 0.25, maxIter = 10)
          ...
          if (inherits(e, "TimeoutException")) {
            message(sprintf("Bootstrap iteration %d failed due to TIMEOUT (severity model).", b))
          } else {
            message(sprintf("Bootstrap iteration %d failed (severity model): %s", b, e$message))
          }
          # Store NA if any failure occurs
          coef_sev_BS[-c(1,2), b] <- NA
        })
    ...
    print("MC dataset lacks 1/2 FRI Scores.")
  }
