...
if(file.exists(path))
{
  ...
  if(all(c(0,1,2) %in% unique(temp$FRI_Score))){
    ...
    setwd(paste0("/blue/somnath.datta/shoumisarkar/Fluorosis/Results/06_simulation/PowerStudy/N", N, "/", "presence", "/",
                 corstr_pres, ",", corstr_sev, "/age", age))
    ...
    setwd(paste0("/blue/somnath.datta/shoumisarkar/Fluorosis/Results/06_simulation/PowerStudy/N", N, "/", "severity", "/",
                 corstr_pres, ",", corstr_sev, "/age", age))
