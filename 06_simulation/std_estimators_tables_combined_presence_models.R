...
for(N in c(30, 50, 200))
{
  ...
  for(age in ages)
  {
    ...
    stdest = apply(stdest_list[[age_ind]], MARGIN = 1, function(x){mean(x, na.rm = T)  })/sqrt(N) #[-c(1:n_intercept)]
    ...
    temp_df = data.frame(Variable = names(SE_JS_stdest),
                         StdEstimate = stdest[-c(1:3)],
                         SE = SE_stdest[-c(1:3)],
    ...
    temp_df$StdBias = temp_df$StdEstimate - temp_df$Target
    temp_df$StdMSE = (temp_df$StdBias)^2 + (temp_df$SE)^2
    
    temp_df$JS_StdBias = temp_df$JS_StdEstimate - temp_df$Target
    temp_df$JS_StdMSE = (temp_df$JS_StdBias)^2 + (temp_df$JS_SE)^2
    
    temp_df = temp_df[,c("Variable", 
                         "StdEstimate", "StdBias", "SE", "StdMSE", 
                         "JS_StdEstimate", "JS_StdBias", "JS_SE", "JS_StdMSE")]
    
    temp_df[-1] <- round(as.numeric(as.character(unlist(temp_df[-1]))), 3)  # Round the numeric columns
    
    vec_avgMSE = rep("", times=length(colnames(temp_df)))
    vec_avgMSE[1] = "Average MSE"
    vec_avgMSE[which(colnames(temp_df) %in% c("StdMSE", "JS_StdMSE"))] = 
      round(c(mean(temp_df$StdMSE), mean(temp_df$JS_StdMSE)), 3) 
    
    temp_df = rbind(temp_df, vec_avgMSE)
  ...
  #save(est_list, file=paste0("combined_presence_aggreg_est_", model,"_", corstr_pres, ",", corstr_sev, ".Rdata"))
  #save(stdest_list, file=paste0("combined_presence_aggreg_stdest_", model,"_", corstr_pres, ",", corstr_sev, ".Rdata"))
  #save(JS_stdest_list, file=paste0("combined_presence_aggreg_JS_stdest_", model,"_", corstr_pres, ",", corstr_sev, ".Rdata"))
  ...
  write.xlsx(summary_list, file = paste0("summarytable_std_estimates_", model,"_presence_", corstr_pres, ",", corstr_sev, "_N_", N, ".xlsx"), 
