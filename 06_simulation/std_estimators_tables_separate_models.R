...
for(model in models)
{
  ...
  for(N in c(30, 50, 200))
  {
    ...
    for(age in ages)
    {
      ...
      stdest = apply(stdest_list[[age_ind]], MARGIN = 1, function(x){mean(x, na.rm = T)  })[-c(1:n_intercept)]/sqrt(N)
      JS_stdest = apply(JS_stdest_list[[age_ind]], MARGIN = 1, function(x){mean(x, na.rm = T)  })[-c(1:n_intercept)]/sqrt(N)
      
      SE_stdest = apply(stdest_list[[age_ind]], MARGIN = 1, function(x){sd(x, na.rm = T)  })[-c(1:n_intercept)]/sqrt(N)
      SE_JS_stdest = apply(JS_stdest_list[[age_ind]], MARGIN = 1, function(x){sd(x, na.rm = T)  })[-c(1:n_intercept)]/sqrt(N)
