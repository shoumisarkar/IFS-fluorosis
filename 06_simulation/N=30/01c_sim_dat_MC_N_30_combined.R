...
obj = load(paste0("/blue/somnath.datta/shoumisarkar/Fluorosis/Results/05a_combined_presence_modelling/exchangeable,exchangeable/whole_data_based/coefs_pres_age",age, ".RData"))
...
if(length(unique(temp$FRI_Score))!=4)
{
  ...
  pres_obj = load(path_pres) ; assign("init_pres_coef", get(pres_obj))
  init_pres_coef = init_pres_coef$Estimates
  ...
  sev_obj = load(path_sev) ; assign("init_sev_coef", get(sev_obj))
  init_sev_coef = init_sev_coef$Estimates
  
  
  #Fit the GEE:
  
  MC_output = fit_GEE_combined(dat=MC_dataset, corstr_pres = corstr_pres,
                               corstr_sev = corstr_sev, init_pres_coef = init_pres_coef,
                               init_sev_coef = init_sev_coef, kappa=0.25, maxIter = 100)
  ...
  #Read in the JK estimates from the separate models:
  path_pres_JK = paste0("/blue/somnath.datta/shoumisarkar/Fluorosis/Results/06_simulation/N", N, "/", "presence", "/",
                        corstr_pres, ",", corstr_sev, "/age", age, "/", "coef_pres_JK_MC_", mc_seed, ".Rdata")
  pres_JK_obj = load(path_pres_JK)
  assign("pres_coef_JK", get(pres_JK_obj))
  
  
  path_sev_JK = paste0("/blue/somnath.datta/shoumisarkar/Fluorosis/Results/06_simulation/N", N, "/", "severity", "/",
                        corstr_pres, ",", corstr_sev, "/age", age, "/", "coef_sev_JK_MC_", mc_seed, ".Rdata")
  sev_JK_obj = load(path_sev_JK)
  assign("sev_coef_JK", get(sev_JK_obj))
  ...
  for(ID in unique(MC_dataset$SUBJECT_ID))
  {
    ...
    if(length(unique(MC_dataset_JK$FRI_Score)) == 4)
    {
      MC_output_JK = 
        fit_GEE_combined(dat=MC_dataset_JK, corstr_pres = corstr_pres,
                         init_pres_coef = as.vector(pres_coef_JK[-c(1,3,4), count]),
                         init_sev_coef = as.vector(sev_coef_JK[-c(1,2), count]),
                         kappa=0.25, maxIter = 100)
      ...
      coef_pres_JK[ -c(1,3,4), count] = MC_output_JK$pres_coefs$Estimates
      coef_sev_JK[ -c(1,2), count] = MC_output_JK$sev_coefs$Estimates
    }
