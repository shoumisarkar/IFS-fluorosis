...
corstr_pres = "exchangeable" #"jackknifed" # "exchangeable" #"independence"
corstr_sev = "independence" #"jackknifed" # "exchangeable"  #"independence" #"exchangeable"
...
if(length(unique(temp$FRI_Score))!=4)
{
  ...
  MC_output = fit_GEE_presence(dat=MC_dataset, corstr_pres = corstr_pres, kappa=0.25, maxIter = 100)
  ...
  for(ID in unique(MC_dataset$SUBJECT_ID))
  {
    ...
    {
      MC_output_JK = 
        fit_GEE_presence(dat=MC_dataset_JK, corstr_pres = corstr_pres, kappa=0.25, maxIter = 100)
