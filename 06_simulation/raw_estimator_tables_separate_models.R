...
for(model in models)
{
  ...
  for(N in c(30, 50, 200))
  {
    ...
    for(age in ages)
    {
      ...
      # 
      # SE_stdest = apply(stdest_list[[age_ind]], MARGIN = 1, function(x){sd(x, na.rm = T)  })[-c(1:n_intercept)]
    ...
    save(summary_list, file=paste0("summarytable_raw_est_", model,"_", corstr_pres, ",", corstr_sev, ".Rdata"))
